...
namespace TempDetect
{
        ...
        public class MewtocolClass
        {
            ...
            public String RCP(string d, int n)
            {
                ...
                sendData = "%" + station.ToString().PadLeft(2, '0') + "#" + "RCP" + n.ToString();
            ...
            public String WCP(string d, int n, string staus)
            {
                ...
                sendData = "%" + station.ToString().PadLeft(2, '0') + "#" + "WCP" + n.ToString();
                for (byte by = address; by < (address + n); by++)
                {
                    byte[] bs1 = new byte[1];
                    bs1[0] = by;                                   //by为最后一位的值                                          
            ...
            **************************************************************************************/
            ...
            public String RCC(string d, int n)
            {
                ...
                sendData = "%" + station.ToString().PadLeft(2, '0') + "#" + "RCC" + d[0] + f1 + f2;
                //s = sendData + "**\r";                             //不带校验码
                s = sendData + BCC(sendData) + "\r\n";                 //带校验码
                return s;
            }
            ...
            public String WCC(string d, int n, short[] inData)
            {
                ...
                //拼接写入字符串
                //命令 % 站号 # 读写命令                   
                sendData = "%" + station.ToString().PadLeft(2, '0') + "#" + "WCC" + d[0] + f1 + f2 + e;
                // s = sendData + "**\r";                             //不带校验码
                s = sendData + BCC(sendData) + "\r\n";                 //带校验码
                return s;
            }
            ...
            public String RD(string d, int n)
            {
                ...
                if (n <= 25)
                {
                    ...
                    sendData = "%" + station.ToString().PadLeft(2, '0') + "#" + "RD" + d[0] + f1 + f2;
                }
                else
                {
                    sendData = "<" + station.ToString().PadLeft(2, '0') + "#" + "RD" + d[0] + f1 + f2;        //读取字超过25，协议头改为"<"    
                }
                // s = sendData + "**\r";                             //不带校验码
                s = sendData + BCC(sendData) + "\r\n";                 //带校验码
                return s;
            ...
            /****************************读写数据长度的辅助说明**********************************************
             ...
             以上 即最多返回100字节的数据，读一个字需要4字节，那么标准协议格式最多可以读取25个字的数据。
            ...
            public String WD(string d, int n, short[] inData)
            {
                ...
                //% # R D 数据代码（1字符）起始数据编码(5 字符) 结束数据编码(5 字符)   BCC(H) BCC(L) CR               
                sendData = "%" + station.ToString().PadLeft(2, '0') + "#" + "WD" + d[0] + f1 + f2 + e;
                // s = sendData + "**\r";                             //不带校验码
                s = sendData + BCC(sendData) + "\r\n";                 //带校验码
                return s;
            }
